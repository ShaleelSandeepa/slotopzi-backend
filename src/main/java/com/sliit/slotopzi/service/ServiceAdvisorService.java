...
import com.sliit.slotopzi.dto.response.serviceadvisor.GetAllRepairInstanceResponse;
import com.sliit.slotopzi.enums.*;
...
//        Long vehicle=vehicleRepository.findVehicleIdByVin(addNewRepairsRequest.getVin());
//        Long serviceAdvisor=staffRepository.findStaffIdByUserDataId(addNewRepairsRequest.getUserId());
...
//        Repair newRepair= new Repair();
//        newRepair.setPaymentType(addNewRepairsRequest.getPaymentType());
//        newRepair.setVehicleId(vehicle);
//        newRepair.setStaffStaffId(serviceAdvisor);
//        newRepair.setStatus(RepairStatus.ONGOING);

        System.out.println("newRepair : " + newRepair);

        repairRepository.save(newRepair);
//        slotRepository.update();
        System.out.println("repairRepository");
//        Repair repair=repairRepository.findByStatusAndAndVehicle(RepairStatus.ONGOING,vehicle);
        List<Repair> repairs = repairRepository.findByStatusAndAndVehicle(RepairStatus.ONGOING, vehicle);

//        System.out.println("getRepairId: "+repair.getRepairId());
//        return repairs.get(repairs.size()).getRepairId();
        return repairs.stream()
                .max(Comparator.comparing(Repair::getRepairAddedDate)) // Replace with your timestamp field
                .map(Repair::getRepairId) // Extract the ID
                .orElseThrow(() -> new NoResultException("No repairs found for the specified status and vehicle."));
    }
